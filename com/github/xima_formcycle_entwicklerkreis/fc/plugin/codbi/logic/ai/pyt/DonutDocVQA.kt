...
/**
 * The specific Translator logic for Donut DocVQA.
 * Handles resizing, prompt generation, and the autoregressive decoding loop.
 */
class DocVQATranslator(private val modelDir: String, protected val log:(importance: AI.LogLevel, toLog: String) -> Unit) : Translator<kotlin.Pair<DjlImage, String>, String> {
...
/**
 * The Plugin Action Entry Point.
 * Now simplified thanks to the Base Class.
 */
...
override val resModelBaseURL = "https://huggingface.co/Callari/donut-docvqa/resolve/main"
    ...
    override val resModelFiles = listOf("donut-docvqa.pt","config.json","tokenizer.json","added_tokens.json","special_tokens_map.json")
    ...
    override fun execute(params: IPluginServletActionParams): IPluginServletActionRetVal {
        ...
        val jsonParts = finalResults.map { (filename, fields) ->
            val fieldJson = fields.entries.joinToString(",") { (k, v) ->
                "\"$k\":\"${v.replace("\"", "").replace("\n", " ")}\""
            }
            "\"$filename\": { $fieldJson }"
        }
