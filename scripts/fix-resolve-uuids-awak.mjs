...
// The AIWorkflowAssistant uses 8-space indentation for the function body
const oldBody = `          val tableNames = listOf("RESOURCE_PROJECT", "FILE_RESOURCE_PROJECT", "FILE_PROJECT")
...
if (!c.includes(oldBody)) {
    console.log('Old function body not found - trying trimmed version');
    // Trim whitespace for comparison
    const lines = c.split('\n');
    console.log('Looking for resolveProjectFileUuid...');
    for (let i = 0; i < lines.length; i++) {
        if (lines[i].includes('resolveProjectFileUuid')) {
...
const newBody = `          var foundUuid: UUID? = null
...
console.log('AIWorkflowAssistant.kt resolveProjectFileUuid updated');
