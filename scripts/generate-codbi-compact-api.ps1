...
$compactDescOverrides = @{
  ...
  "HTML.Panel"           = "Applicable on any element to wrap it in a collapsible accordion/panel widget. PREFERRED: 'Standard-Panel' / 'aufklappbares/collapsible panel' on a FIELDSET = XFieldSet + CodBi_HTML_Panel_Standard CSS class + legend property (never data-cb-func=html.panel on a fieldset). On an XCONTAINER (div — no legend) use data-cb-func=html.panel and you MUST also set data-cb-generateheader='true' and a NON-EMPTY data-cb-autoheadertitle: the autoheadertitle is the ONLY source of the container panel's header text — use the title from the prompt, or if the prompt names no title, INVENT a short, meaningful title from the container's purpose/content in the form's language. When the prompt does not describe the header's look, ALSO generate restrained header CSS via the data-cb-* CSS parameters (every value is a RAW CSS declaration list WITHOUT braces, e.g. background:#eef2f7; padding:10px 14px; border-radius:6px;): data-cb-cssheaderfolded (base look while FOLDED — set it to the SAME CSS as data-cb-cssheaderunfolded so a folded panel looks identical), data-cb-cssheaderunfolded (base look while unfolded), data-cb-cssheaderhover, data-cb-cssheaderactive, and a fold indicator via data-cb-cssafterheadercontent (folded) plus data-cb-cssafterheadercontentunfolded (unfolded) — e.g. a downward arrow when folded and an upward arrow when unfolded. Follow the prompt exactly when it does describe the look. FOLDED-STATE HEADER STYLING: data-cb-cssheaderunfolded styles the header ONLY while the panel is unfolded (a panel that STARTS folded shows an unstyled header until unfolded, because folding restores the header's original inline style). For the folded state use data-cb-cssafterheader / data-cb-cssbeforeheader (plus their ...content strings; unfolded counterparts ...unfolded). The FOLDED header look is now set directly with data-cb-cssheaderfolded (use the SAME CSS as data-cb-cssheaderunfolded); OPTIONALLY you may also emit a state-independent rule on the header class via HTML.CSS: an element with data-cb-func=HTML.CSS and data-cb-css='.CodBi_HTML_Panel_Header { background:#eef2f7; border-radius:6px; padding:10px 14px; font-weight:600; color:#1e293b; cursor:pointer; }' (the header keeps the class CodBi_HTML_Panel_Header in both states). MANDATORY — NEVER SKIP A CONTAINER PANEL'S HEADER STYLING (even when the request does not mention styling): creating/converting an XContainer into a collapsible panel (data-cb-func=html.panel) REQUIRES (a) BOTH HTML.Panel AND HTML.CSS in the need_codbi_details request, (b) data-cb-generateheader='true' + a non-empty data-cb-autoheadertitle, (c) an HTML.CSS element carrying the state-independent rule (data-cb-func=HTML.CSS, data-cb-css='.CodBi_HTML_Panel_Header { background:#eef2f7; border-radius:6px; padding:10px 14px; font-weight:600; color:#1e293b; cursor:pointer; }') unless the prompt specifies a different look, and (d) BOTH HTML.CSS and HTML.Panel listed in the _codbiApplicability applied array. NEVER return codbiVerdict='none' with empty considered/applied for such a request. The same applies when the form ALREADY has container panels without header styling — apply HTML.CSS and report it as considered/applied."
