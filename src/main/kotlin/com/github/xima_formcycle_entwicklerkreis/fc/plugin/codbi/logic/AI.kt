 ...
 *
 * In order to enable the re-usage of images that were already uploaded, this class sets up
 * a cache ([cacheIDedImages]) that is managed by a janitor ([janitorIDedImages]) with a specific expiration time
 * for the images defined ([msExpirationIDedImages]).
 *
...
abstract class AI : IPluginServletAction {
  ...
  protected open fun log( importance: LogLevel, toLog: String, adjenct: String = "", exception: Throwable? = null ) {
    val message = "[[ CodBi / AI${ if( idLogMessages.isEmpty()) "" else " / $idLogMessages$adjenct"} ] $toLog ]"

    when( importance ) {
      LogLevel.INFO     -> logger.info(message)
      LogLevel.WARNING  -> logger.warn(message)
      LogLevel.ERROR    -> {
        if (exception != null) { logger.error(message, exception)} else { logger.error( message )}
      }
    }
  }
  /**
