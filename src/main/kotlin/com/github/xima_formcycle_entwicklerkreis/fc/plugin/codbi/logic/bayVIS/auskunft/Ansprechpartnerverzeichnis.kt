 ...
 * - BayVIS_EP_Ansprechpartnerverzeichnis: Optional specification of where the endpoint for
 *   retrieving the authority directory can be found.
 */
class CodBiBayVISAuskunftAnsprechpartnerverzeichnisAction : IPluginServletAction {
  /** Reflects the result of [CodBiBayVISAuskunftAnsprechpartnerverzeichnisAction]'s validation. */
  class ValidationResult : IPluginInitializeValidationResult {
    /**
     * Stores a [Boolean] specifying whether a [CodBiBayVISAuskunftAnsprechpartnerverzeichnisAction]
     * is valid.
     */
    protected var valid: Boolean = false
    /**
     * Stores the [List] of [String]s that were generated as error message in case a
     * [CodBiBayVISAuskunftAnsprechpartnerverzeichnisAction] is not valid.
     */
    protected var messages: List<String> = listOf("")

    /** @return If this [[CodBiBayVISAuskunftAnsprechpartnerverzeichnisAction]] [valid]. */
    public override fun isValid(): Boolean {
      return valid
    }

    /** @return The [messages]. */
    public override fun getErrorMessages(): List<String> {
      return messages
    }

    /**
     * Constructs this [IPluginServletAction] by setting it's [messages] and specifying whether it
     * is [valid] or not.
     *
     * @param isValid See [valid].
     * @param message The only member for the [messages].
     */
    public constructor(isValid: Boolean, message: String) {
      this.valid = isValid
  ...
  /**
   * Updates the [username], [password], [hrsTillUpdate] and the contact directory into the [buffer]
   * using the configuration-properties **BayVIS_Benutzername** && **BayVIS_Passwort**. Furthermore,
   * credential validity is checked.
   *
   * @return A proper [IPluginInitializeValidationResult] or [null] if everything is fine.
   */
  override fun validateConfigurationData(
      configData: IPluginValidationData
  ): IPluginInitializeValidationResult? {
    // region Don't check if there is either no username or no password available.
    ...
    // endregion Don't check if there is either no username or no password available.
    ...
    return ValidationResult(
        false, result.second!!) // Which IPluginInitializeValidationResult? Create own class?
  }

  /** Returns "CodBi_BayVIS_Auskunft_Ansprechpartnerverzeichnis" */
  override fun getName(): String {
    return "CodBi_BayVIS_Auskunft_Ansprechpartnerverzeichnis"
  }
  ...
  override fun initialize(configData: IPluginInitializeData) {
    ...
    val newURL: String? = configData.properties.getProperty("BayVIS_EP_Ansprechpartnerverzeichnis")
