 ...
 * - BayVIS_EP_Behoerdendetails: Optional specification of where the endpoint for retrieving the
 *   authority directory can be found.
 */
class CodBiBayVISAuskunftBehoerdendetailsAction : IPluginServletAction {
   ...
   * reserved characters are escaped). Guards against a raw request value (e.g. an AI-generated "Amt
   * 44") producing an invalid URI.
   *
  ...
  protected fun retrieveData(id: String): Pair<Int, String?> {
    ...
    try {
      val url: URL = URI(url + pathSegment(id)).toURL()
  ...
  /** Returns "CodBi_BayVIS_Auskunft_Behoerdendetails" */
  override fun getName(): String {
    return "CodBi_BayVIS_Auskunft_Behoerdendetails"
  }
  ...
  override fun initialize(configData: IPluginInitializeData) {
    ...
    val newURL: String? = configData.properties.getProperty("BayVIS_EP_Behoerdendetails")
