  ...
  private fun handleRun(params: IPluginServletActionParams): IPluginServletActionRetVal {
    ...
    val systemPrompt =
        "You are a FORMCYCLE form structure assistant. You receive a partial IPersistJson object " +
            ...
            "EXAMPLE — AI.OCR with Mode=\"print\": the form JSON must contain TWO separate items in the items array — an XUpload with data-cb-func=\"ai.ocr\" and a SEPARATE XTextField with cssclasses [\"CodBi_AI_OCR_Receiver\"]. The TWO-OPTION rule does NOT apply here because the CSS class and the data-cb-func are on DIFFERENT elements:\n" +
            ...
            "CRITICAL — CSS classes and data-cb-func can be on DIFFERENT elements. Example: an XUpload with data-cb-func=\"ai.ocr\" and a separate XTextField with cssclasses [\"CodBi_AI_OCR_Receiver\"] — these are two different elements, each with their own purpose. The CSS class is NOT an alternative to data-cb-func here; it is applied to a different element.\n" +
