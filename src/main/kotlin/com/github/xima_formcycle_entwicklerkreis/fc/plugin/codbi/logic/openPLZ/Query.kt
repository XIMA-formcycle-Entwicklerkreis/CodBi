 ...
 * OpenPLZ REST API (checkout the **[documentation](https://www.openplzapi.org/de/)** for further information about the
 * header-parameter). Unlike the REST-Service returning paginated results, this servlet accumulates all available data
 * and returns it as a single block.
 *
 * Configuration Keys:
 ...
 * - orgaUnit:      The organisational unit to fetch (e.g. /de/ **FederalStates**). This may be **FederalStates**,
 *                  **FederalProvinces** or **Cantons**.
 * - officialKey    The optional key (number) of a state, province or canton to get details from.
 * - detail:        The optional detail to fetch about a certain state, province or canton identified by the
 *                  **officialKey** (not optional if an official key is present). May be Municipalities,
 *                  or Districts.
 * - param1         There may be up to four parameter passed along the request (e.g. **postalCode**, **name**,
 *                  **locality**, **searchTerm**).
 * - param2         There may be up to four parameter passed along the request (e.g. **postalCode**, **name**,
 *                  **locality**, **searchTerm**).
 * - param3         There may be up to four parameter passed along the request (e.g. **postalCode**, **name**,
 *                  **locality**, **searchTerm**).
 * - param4         There may be up to four parameter passed along the request (e.g. **postalCode**, **name**,
 *                  **locality**, **searchTerm**).*/
class CodBiOpenPLZQueryAction : IPluginServletAction {
    ...
    protected fun retrieveData( toRetrieveFrom : String ): Pair<Int, String?> {
        LoggerFactory.getLogger(CodbiFormResourcesPlugin::class.java)
            .info("[[ CodBiOpenPLZQueryAction ] Fetching data from OpenPLZ REST with following URL: $toRetrieveFrom ]");
        ...
        try {
            ...
            buffer[ toRetrieveFrom ]    = "{\"error\": \"Error connecting to OpenPLZ API\"}"
            statusCode                  = 500
        }
    ...
    override fun validateConfigurationData(
        ...
        if (configData.properties.getProperty("BayVIS_UpdateCycle") != null)    hrsTillUpdate   = configData.properties.getProperty("BayVIS_UpdateCycle").toInt()
        if (configData.properties.getProperty("OpenPLZ_URL") != null)           url             = configData.properties.getProperty("OpenPLZ_URL")
        if (configData.properties.getProperty("OpenPLZ_Country") != null)       country         = configData.properties.getProperty("OpenPLZ_Country")
    ...
    override fun initialize(configData: IPluginInitializeData) {
        if (configData.properties.getProperty("OpenPLZ_UpdateCycle") != null)   hrsTillUpdate   = configData.properties.getProperty("OpenPLZ_UpdateCycle").toInt()
        if (configData.properties.getProperty("OpenPLZ_URL") != null)           url             = configData.properties.getProperty("OpenPLZ_URL")
        if (configData.properties.getProperty("OpenPLZ_Country") != null)       country         = configData.properties.getProperty("OpenPLZ_Country")
    }
    /**
    ...
    public override fun execute(p0: IPluginServletActionParams): IPluginServletActionRetVal {
        ...
        val officialSegment = if (!p0.headerMap["X-OfficialKey"].isNullOrBlank())   "/${p0.headerMap["X-OfficialKey"]!!}/${p0.headerMap["X-Detail"]}"
                              else                                                  ""
        val country         = p0.headerMap["X-Country"] ?: ""
        ...
        optionalKeys.forEach { key ->
            ...
            LoggerFactory.getLogger(CodbiFormResourcesPlugin::class.java)
                .info("[[ CodBiOpenPLZQueryAction ] Param: $value ]");
            if (!value.isNullOrBlank()) {
                val encodedValue = URLEncoder.encode(value.replaceFirst("-","=").replace(" ",""), StandardCharsets.UTF_8.toString()).replace("%3D","=").replace("%2B","%20")

                queryParams.add(encodedValue)}
        }

        val baseQueryString = if ( queryParams.isNotEmpty()) "?" + queryParams.joinToString("&")
                              else                           ""
        // endregion Build the Base URL.
        do {
            val pageQueryParam  = if (baseQueryString.isEmpty())    "?page=$currentPage&pageSize=50"
                                  else                              "$baseQueryString&page=$currentPage&pageSize=50"
            val pagedUrl        = fullPath + pageQueryParam
            val finalBufferKey  = pagedUrl
            val isBufferValid   = buffer.containsKey(finalBufferKey) && (System.currentTimeMillis() - lastContact) / 3600000 < hrsTillUpdate
            ...
            if( !isBufferValid) {
                ...
                if( currentStatusCode !in 200..299 || currentJson == null || currentJson.contains("\"error\"")) {
                    allResults.clear()
                    allResults.append(currentJson ?: "{\"error\": \"Unbekannter Fehler beim Abrufen von Seite $currentPage\"}")
            ...
            if( cleanedJson.isNotEmpty()) {
                ...
                if (objectCount < 50)   hasMorePages = false
                else                    currentPage++
