...
- AI.OCR: Applicable on an XUpload field to extract text from uploaded images/PDFs via OCR and optionally validate the extracted content against a regex pattern. CRITICAL — For content validation on uploads, use AI.OCR's Pattern parameter — do NOT apply HTML.Input.REGEX to XUpload fields (REGEX is for XTextField only). CRITICAL — When the Pattern contains curly braces (e.g. regex quantifiers like {2,}) that would be misinterpreted as Element Placeholders, prefix the value with "^" to mark it as raw (the "^" is stripped at runtime and all curly braces are safely escaped from EP detection). CRITICAL — ALWAYS set data-cb-Mode. Use "extract" for plain text extraction (returns the full extracted text), "verify" for content validation against a Pattern, "print" for server-side printing. CRITICAL — To activate the CodBi_AI_OCR_Receiver CSS class and other AI-related standard configurations, include {"id":"AI","targets":[]} in the _codbiApplicability.applied array. Do NOT set data-cb-Field — instead, tag the target text field with the CSS class "CodBi_AI_OCR_Receiver" in its cssclasses array and activate the AI standard configuration.
  ...
  - Mode: The mode of operation. CRITICAL — ALWAYS set data-cb-Mode. "extract" for plain text extraction (returns the full extracted text to the CodBi_AI_OCR_Receiver-tagged field), "verify" for content validation against the Pattern (upload only valid if extracted text matches), "print" for server-side document printing without extraction.
