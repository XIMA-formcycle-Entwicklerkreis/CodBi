...
- AI.OCR: Applicable on an XUpload field to extract text from uploaded images/PDFs via OCR and optionally validate the extracted content against a regex pattern. CRITICAL — For content validation on uploads, use AI.OCR's Pattern parameter — do NOT apply HTML.Input.REGEX to XUpload fields (REGEX is for XTextField only). CRITICAL — When the Pattern contains curly braces (e.g. regex quantifiers like {2,}) that would be misinterpreted as Element Placeholders, prefix the value with "^" to mark it as raw (the "^" is stripped at runtime and all curly braces are safely escaped from EP detection). When the prompt requires extracting a document's text as a whole (no field-specific extraction, no validation), use Mode="print" and create an XTextField (or XTextArea) with the CSS class "CodBi_AI_OCR_Receiver" in its cssclasses array to receive the extracted text — this CSS class MUST be added to the generated text field. When validating content against a Pattern, use Mode="verify" and ALWAYS set data-cb-Pattern, data-cb-RegExFlags, data-cb-WrongFileMessage, data-cb-InvalidImageText.
  - Field: CSS-Class-Selector for the field receiving the OCR output (e.g., '.tfExtractedText'). Use dot-prefixed class selector based on the target element's name. Do NOT use an ID selector. Omit if not needed.
  ...
  - Mode: The mode of operation. CRITICAL — ALWAYS set data-cb-Mode. "print" for plain text extraction (returns the full extracted text — you MUST generate an XTextField or XTextArea with the CSS class "CodBi_AI_OCR_Receiver" in its cssclasses array; this CSS class is REQUIRED on the generated text field), "verify" for content validation against a Pattern.
