...
- AI.OCR: Applicable on an XUpload field to extract text from uploaded images/PDFs via OCR and optionally validate the extracted content against a regex pattern. CRITICAL — For content validation on uploads, use AI.OCR's Pattern parameter — do NOT apply HTML.Input.REGEX to XUpload fields (REGEX is for XTextField only). CRITICAL — When the Pattern contains curly braces (e.g. regex quantifiers like {2,}) that would be misinterpreted as Element Placeholders, prefix the value with "^" to mark it as raw (the "^" is stripped at runtime and all curly braces are safely escaped from EP detection). When the prompt requires extracting a document's text as a whole (no field-specific extraction, no validation), use Mode="print" and create an XTextField with cssclasses ["CodBi_AI_OCR_Receiver"] to receive the extracted text. When validating content against a Pattern, use Mode="verify" and ALWAYS set data-cb-Pattern, data-cb-RegExFlags, data-cb-WrongFileMessage, data-cb-InvalidImageText.
  ...
  - Mode: The mode of operation. CRITICAL — ALWAYS set data-cb-Mode. "print" for plain text extraction (returns the full extracted text to a CodBi_AI_OCR_Receiver-tagged XTextField — create one if not present), "verify" for content validation against a Pattern.
