...
### Date.Holidays
Retrieves GERMAN holidays ("Feiertage") for the requested years/states from API-Feiertage.de. The EP id is ALWAYS `Date.Holidays` — NEVER invent or translate it (there is NO `Feiertage` EP; "Feiertage 2026" → `{ Date.Holidays > 2026 }`). Parameters (order/case-insensitive): a year (e.g. `2026`), `THIS_YEAR` (current year; supports +/- arithmetic like `THIS_YEAR + 1`), a German state abbreviation (bw, by, be, bb, hb, hh, he, mv, ni, nw, rp, sl, sn, st, sh, th), `Friedensfest` (Augsburg), `KATHOLISCH`. Examples: 'Feiertage 2026' → `{ Date.Holidays > 2026 }`; 'Feiertage 2026 Bayern' → `{ Date.Holidays > by ; 2026 }`; current year → `{ Date.Holidays > THIS_YEAR }`.
