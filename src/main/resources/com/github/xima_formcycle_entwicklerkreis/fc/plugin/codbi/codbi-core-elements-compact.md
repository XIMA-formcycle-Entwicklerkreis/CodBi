...
### Date.Time.Join.Span
Validates a BEGIN date+time pair against an END date+time pair. Tag the CONTAINER that holds the four fields with data-cb-func="Date.Time.Join.Span" — the tagged element is the scope in which the fields are searched. All four fields may either be referenced via the optional CSS-Selector-Parameters (data-cb-Begin, data-cb-BeginTime, data-cb-End, data-cb-EndTime) or, if those aren't set, via the CSS-Classes they're tagged with (CodBi_Date_Time_Join_Span_Begin, CodBi_Date_Time_Join_Span_Begin_Time, CodBi_Date_Time_Join_Span_End, CodBi_Date_Time_Join_Span_End_Time) — the Parameters take precedence over the Classes and are REQUIRED when more than one span exists in the form or when fields lie outside the tagged element. The CSS-Classes are looked up within the tagged element (fields inside a nested element tagged with Date.Time.Join.Span are SKIPPED, so an outer span never resolves an inner span's fields); the CSS-Selector-Parameters first within it, then across the whole document — the begin and end fields may thus reside in different containers. If any of the four fields cannot be resolved, the functionality simply does not take effect (no exception). The joined end datetime must be higher than the joined begin datetime. When the joined end datetime is lower or equal to the joined begin datetime (or, equivalently, the joined begin datetime is higher or equal to the joined end datetime — equality is NOT permitted), exactly the date- or time-field that just changed is invalidated (data-cb-msginvalid). This is NOT Date.Frame/Time.Frame (those compare dates or times SEPARATELY) and NOT Date.Time.Join (that only merges one date+time pair) — it validates a full begin/end datetime span across four fields.
