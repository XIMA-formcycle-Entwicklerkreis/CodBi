...
- FOLDED-STATE HEADER + STATE-INDEPENDENT HEADER CSS (both together): data-cb-cssheaderunfolded applies ONLY while the panel is UNFOLDED — a panel that STARTS folded shows an unstyled header until unfolded. So (1) for the FOLDED state use data-cb-cssafterheader / data-cb-cssbeforeheader (+ their ...content strings; unfolded variants ...unfolded), and (2) ALSO emit a state-independent rule via the HTML.CSS functionality — an element with data-cb-func="HTML.CSS" and data-cb-css=".CodBi_HTML_Panel_Header { background:#eef2f7; border-radius:6px; padding:10px 14px; font-weight:600; color:#1e293b; cursor:pointer; }" — so the header looks IDENTICAL when folded and unfolded (the header carries the class CodBi_HTML_Panel_Header in both states). Prefer this state-independent rule for the base look.
- MANDATORY — NEVER SKIP A CONTAINER PANEL'S HEADER STYLING (even when the request does not mention styling): creating/converting an XContainer/XContainerInvisible into a collapsible panel (data-cb-func=html.panel) REQUIRES (a) `HTML.Panel` AND `HTML.CSS` in the need_codbi_details request, (b) data-cb-generateheader="true" + a non-empty data-cb-autoheadertitle, (c) the header CSS ON THE PANEL ITSELF via html.panel's OWN CSS parameters on that container — data-cb-cssheaderfolded (base look while FOLDED — set it to the SAME value as data-cb-cssheaderunfolded) and data-cb-cssheaderunfolded (base look while unfolded, e.g. `background:#eef2f7; border:1px solid #cbd5e1; border-radius:6px; padding:10px 14px; font-weight:600; color:#1e293b; cursor:pointer;`), data-cb-cssheaderhover, data-cb-cssheaderactive, data-cb-cssafterheadercontent (folded arrow) / data-cb-cssafterheadercontentunfolded (unfolded arrow) — every value a RAW CSS declaration list WITHOUT braces; the folded header is styled directly by data-cb-cssheaderfolded, so the HTML.CSS element is OPTIONAL extra resilience, and (d) `HTML.Panel` AND `HTML.CSS` listed in the `_codbiApplicability` `applied` array. NEVER return `codbiVerdict:"none"` with empty considered/applied for such a request.
