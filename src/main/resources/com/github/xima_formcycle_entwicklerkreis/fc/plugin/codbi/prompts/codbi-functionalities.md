...
CRITICAL — THE HEADER CSS OF A CONTAINER PANEL BELONGS ON THE CONTAINER (html.panel's own CSS parameters — NOT a separate element): a panel built with `data-cb-func=html.panel` on an XContainer carries its header styling in the SAME element's `attributes`. Required parameters (all optional individually, but you MUST set a base look + a fold indicator unless the prompt specifies the look): `data-cb-cssheaderunfolded` (the base header look applied while the panel is UNFOLDED — the main styling slot; e.g. `background:#eef2f7; border:1px solid #cbd5e1; border-radius:6px; padding:10px 14px; font-weight:600; color:#1e293b; cursor:pointer;`), `data-cb-cssheaderhover` (hover feedback), `data-cb-cssheaderactive` (pressed effect), `data-cb-cssafterheadercontent` (fold indicator while FOLDED, e.g. a down arrow) and `data-cb-cssafterheadercontentunfolded` (while UNFOLDED, e.g. an up arrow). Every value is a RAW CSS DECLARATION LIST WITHOUT braces and WITHOUT a selector. IMPORTANT — html.panel has NO folded base-header parameter (`data-cb-cssheaderunfolded` applies only while unfolded; when folded the header keeps its original inline style), so to also style the header while FOLDED emit the separate state-independent `HTML.CSS` element (`.CodBi_HTML_Panel_Header { ... }`) IN ADDITION.

CRITICAL — FOLDABLE PANEL ON AN XCONTAINER NEEDS data-cb-autoheadertitle (invent the title if the prompt names none): to make an XContainer/XContainerInvisible collapsible you MUST use data-cb-func=html.panel (a panel CSS class does NOT work on a container). Because a container has NO 'legend', the panel header text comes ONLY from data-cb-autoheadertitle — an empty/omitted autoheadertitle renders a header with NO text. If the prompt names the panel/section title, use that text. If the prompt does NOT state a title (e.g. "mach alle Bereiche klappbar" / "make all areas collapsible" without naming them), you MUST INVENT a short, meaningful title for each container panel yourself, derived from its purpose or its contained fields (e.g. a container holding first/last name, e-mail and phone → "Kontaktdaten"), written in the form's language, and set data-cb-autoheadertitle="<that title>". NEVER emit data-cb-func=html.panel on a container with an empty or missing data-cb-autoheadertitle — that is a FAIL. (On an XFieldSet the 'legend' provides the title automatically, so there data-cb-autoheadertitle is only needed to override the legend.)
...
- data-cb-autoheadertitle="<title>" — the header text — REQUIRED and NON-EMPTY. Use the panel/group title from the prompt; when the prompt does not name one, INVENT a short, meaningful title from the container's purpose/content (in the form's language) and use that. A container has no 'legend', so an empty title renders a header with no text. (On an XFieldSet the 'legend' is the title; set this only to override it.)
...
- data-cb-autoheadertitlesupplementsspacer="<separator>" - the separator inserted between the panel's auto header title and the values of the "supplement" fields (default " / "). Use it together with the CSS class CodBi_HTML_Panel_AutoHeaderTitle_Supplement whenever field VALUES shall be appended to the panel header title - e.g. "Von und Bis sollen dem Oeffnungszeitentitel, durch ein - getrennt, hinzugefuegt werden.": put cssclasses=["CodBi_HTML_Panel_AutoHeaderTitle_Supplement"] on EACH such field (the field must be a DIRECT child of the panel, never inside an intervening XFieldSet/XContainer), set data-cb-autoheadertitlesupplementsspacer on the PANEL to the requested separator EXACTLY as the user gave it: FORMCYCLE trims surrounding whitespace from the attribute, so to get a separator WITH spaces encode them as %20 (URL) or \s - e.g. for "durch ein - getrennt" write "%20-%20" (renders " - "), while a bare "-" renders without spaces, and keep data-cb-autoheadertitle as the BASE title ONLY (e.g. "Oeffnungszeiten") - NEVER put [%fieldName%] placeholders of the supplement fields into data-cb-autoheadertitle. The field values are appended automatically at runtime, separated by the spacer (multiple supplements are additionally separated by ",").

FOLDED-STATE HEADER STYLING (important asymmetry): `data-cb-cssheaderunfolded` styles the header ONLY while the panel is UNFOLDED — a panel that STARTS folded shows an unstyled header until it is unfolded (on folding, the header's ORIGINAL inline style is restored). Therefore:
- For the FOLDED state use `data-cb-cssheaderfolded` (the folded base look — set it to the SAME CSS as `data-cb-cssheaderunfolded`) plus the header pseudo-element parameters: `data-cb-cssafterheader` / `data-cb-cssbeforeheader` (plus their content strings `data-cb-cssafterheadercontent` / `data-cb-cssbeforeheadercontent`); their UNFOLDED counterparts are `data-cb-cssafterheader[content]unfolded` / `data-cb-cssbeforeheader[content]unfolded`. `data-cb-cssrequiredfields` / `data-cb-cssrequiredfieldscontent` style the header when it contains a validation-sensitive field.
- The FOLDED header look is set directly with `data-cb-cssheaderfolded` (use the SAME CSS as `data-cb-cssheaderunfolded`). OPTIONALLY you may ALSO emit a state-independent CSS rule on the header class `.CodBi_HTML_Panel_Header` via the HTML.CSS functionality — an element with `data-cb-func="HTML.CSS"` and `data-cb-css=".CodBi_HTML_Panel_Header { background:#eef2f7; border-radius:6px; padding:10px 14px; font-weight:600; color:#1e293b; cursor:pointer; }"`.

HEADER STYLING & FOLD INDICATOR — the available HTML.Panel data-cb-* CSS parameters. EVERY CSS value here is a RAW CSS DECLARATION LIST — no surrounding braces, no selector (e.g. `background:#eef2f7; padding:10px 14px; border-radius:6px;`), NEVER a `{ ... }` object and NEVER a full `selector { ... }` rule:
- data-cb-cssheaderfolded — CSS applied to the header while the panel is FOLDED. Set it to the SAME CSS as data-cb-cssheaderunfolded so a folded panel looks identical to an unfolded one (without it a folded header falls back to its bare style).
- data-cb-cssheaderunfolded — CSS applied to the header while the panel is UNFOLDED (the main header look; panels start unfolded).
- data-cb-cssheaderhover — header CSS on :hover (built-in default `color: darkorange ;`).
- data-cb-cssheaderactive — header CSS on :active (built-in default `scale : .9 ;`).
- data-cb-dcssheaderunfolded — header CSS while unfolded in DARK mode.
- data-cb-cssbeforeheader / data-cb-cssafterheader — the header's ::before / ::after CSS (folded state).
- data-cb-cssbeforeheadercontent / data-cb-cssafterheadercontent — the ::before / ::after CONTENT (a plain string, e.g. a fold arrow " ▾"), NOT CSS.
- data-cb-cssbeforeheadercontentunfolded / data-cb-cssafterheadercontentunfolded (and the ...headerunfolded variants) — the same for the UNFOLDED state.
- data-cb-cssanimfadeinpanel — the body of the unfold @keyframes (e.g. `from { opacity:0; } to { opacity:1; }`), plus data-cb-cssanimfadeinpanelduration (e.g. `0.3s`) and data-cb-cssanimfadeinpaneleasing (e.g. `ease-in-out`).
- data-cb-autoheaderlevel — heading level 1-6 wrapping the auto-generated header text.
- data-cb-scrollblock / data-cb-scroll / data-cb-scrolltotop — scroll behaviour when the panel unfolds.

CRITICAL — GENERATE A PROPER HEADER LOOK WHEN THE PROMPT SPECIFIES NONE: a plain html.panel on an XContainer has almost no base header CSS and looks spartan. When the prompt asks for foldable containers on XContainer elements and does NOT say how the header shall look, GENERATE tasteful, self-consistent header CSS YOURSELF via the parameters above: set data-cb-cssheaderunfolded to a clean base look (background, border and/or border-radius, padding, font-weight/color, cursor:pointer, width:100%, text-align:left), data-cb-cssheaderhover for subtle hover feedback, data-cb-cssheaderactive for a small pressed effect, and a fold indicator via data-cb-cssafterheadercontent (folded, e.g. " ▾") plus data-cb-cssafterheadercontentunfolded (unfolded, e.g. " ▴"). Keep it restrained and readable (sufficient text/background contrast). When the prompt DOES describe the header look, follow it exactly instead of inventing. (XFieldSet panels use the CodBi_HTML_Panel_* classes — they are already styled by the class and need NO such CSS parameters.)
...
CRITICAL — A "QUELLE" / "DATENQUELLE" (a Formcycle DATASOURCE) IS **NOT** AN EP — do NOT use HTML.Select.Injection for it: when the request names a data source configured in the Formcycle backend ("Quelle X" / "Datenquelle X" / "the source X" / "aus der Quelle ...", typically with a column: "Spalte 1 in der Quelle X"), that IS a Formcycle DATASOURCE, not an element placeholder. Bind it with the XSelect's OWN properties — `datasource` (the datasource name, used AS-IS), the 1-based `dstextidx` (option text) / `dsvalueidx` (option value) / `dstitleidx` (option title), optionally `dstype` (DB, CSV, JSON, XML, LDAP, USER, PLUGIN) and `ds_rendercolattr` — and leave `options` EMPTY ([]). NEVER invent an EP for it (e.g. `{ Staatsangehoerigkeit > column1 }` — a datasource name is NOT an EP id and cannot resolve) and NEVER put it into data-cb-Values. Add NO data-cb-func at all for a datasource select. See the XSelect "Data source" section in the widget reference for the exact JSON.
