...
- `dstextidx` — the 1-BASED column whose values become the option TEXT.
- `dsvalueidx` — the 1-BASED column whose values become the option VALUE (submitted value).
- `dstitleidx` (optional) — the 1-BASED column for the option title/tooltip.
...
COLUMN NUMBERS ARE 1-BASED ("Spalte 1" = the FIRST column). One named column → set BOTH `dstextidx` and `dsvalueidx` to it; two named columns → map each to its own property. The designer defaults are dstextidx "1" and dsvalueidx "2".
Example — "Füge eine Auswahl hinzu, die die Spalte 1 in der Quelle Staatsangehörigkeiten als Option anbietet.": `{"className":"XSelect","properties":{"name":"selStaatsangehoerigkeit","label":"Staatsangehörigkeit","datasource":"Staatsangehörigkeiten","dstextidx":"1","dsvalueidx":"1","options":[]}}`.
