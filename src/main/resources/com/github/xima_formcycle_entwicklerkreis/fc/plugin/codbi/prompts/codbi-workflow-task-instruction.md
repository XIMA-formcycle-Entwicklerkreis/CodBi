...
WORKED EXAMPLE — repeatable opening-hours rows that must ALL appear in an email body: build ONE server attribute per submission and reference it in the FC_EMAIL body (German request "Neue Öffnungszeiten per Mail an Testamt@Ansbach.de" with the repeatable container holding selWeekday/tfFrom/tfTo):
  {"nodeType":"FC_EXPERIMENT","nodeParams":{"_childNodes":[{"nodeType":"FC_FOR_EACH_LOOP","nodeParams":{"fieldTechnicalId":"selWeekday","sourceType":"FORM_FIELD_REPETITIONS","_childNodes":[{"nodeType":"FC_WRITE_FORM_RECORD_ATTRIBUTES","nodeParams":{"attributes":[{"name":"openingHours","value":"[%$RECORD_ATTR.openingHours%][%selWeekday%]: [%tfFrom%] - [%tfTo%]\n"}]}}]}},{"nodeType":"FC_EMAIL","nodeParams":{"to":"Testamt@Ansbach.de","from":"Intranet@Ansbach.de","subject":"Neue Öffnungszeiten","body":"<p>...</p><p>[%$RECORD_ATTR.openingHours%]</p>"}},{"nodeType":"FC_SHOW_TEMPLATE","nodeParams":{"htmlTemplate":"Submission successful"}}],"_handlerChildNodes":[{"nodeType":"FC_SHOW_TEMPLATE","nodeParams":{"htmlTemplate":"Allgemeiner Fehler 2"}}]}}
  EXACT nodeParams: FC_WRITE_FORM_RECORD_ATTRIBUTES ALWAYS uses {"attributes":[{"name":"<key>","value":"<value>"}]} — NEVER "attributeKey". The per-row "value" MUST start with the previous [%$RECORD_ATTR.<key>%] so the rows CONCATENATE (each iteration appends one line); a per-row write WITHOUT that prefix overwrites the attribute and only the LAST row survives. The FC_WRITE_FORM_RECORD_ATTRIBUTES sits INSIDE the FC_FOR_EACH_LOOP's nodeParams._childNodes (runs once per row), never after the loop. For FORM_FIELD_REPETITIONS set the loop's fieldTechnicalId to the technical ID of ONE FIELD inside the repeatable container (e.g. selWeekday) — NOT the container name (e.g. coOpeningHours). LIST FC_WRITE_FORM_RECORD_ATTRIBUTES and FC_FOR_EACH_LOOP in need_workflow_node_details so you receive their exact schemas.
...
EXISTING WORKFLOW STRUCTURE (when present at the end of the prompt) lists the FULL current content of every node above (customParameters, with the same numeric 'id'). Use it to modify existing nodes correctly:
- When you MODIFY/REPLACE an existing FC_EMAIL that sends REPEATABLE-CONTAINER rows, those rows are accumulated by the existing FC_WRITE_FORM_RECORD_ATTRIBUTES (inside its FC_FOR_EACH_LOOP) into a server attribute that the FC_EMAIL body references. ALSO update that FC_WRITE_FORM_RECORD_ATTRIBUTES node (by its numeric id) so its per-row accumulated value includes every field the user now wants per row (e.g. add the Von/Bis fields to the accumulated line), and KEEP the FC_EMAIL body referencing the server attribute.
- NEVER flatten the rows into plain [%fieldName%] placeholders of repeatable fields in the FC_EMAIL body — those resolve to the FIRST row only.
- NEVER invent hidden form fields / helper fields (e.g. a hidden per-row field labeled "… für Mail") to collect repeatable content — the server attribute is the mechanism.
...
- Output ONLY valid JSON (a task object, an array of task objects, {"workflow":[...]}, {"tasks":[...]}, or the delta form {"operations":[...]} where each entry is a task object carrying "operation"/"targetNodeId"/"chainedNodes"). NEVER wrap the workflow inside a form "items" array. No prose, no explanation, no Markdown.
...
- NEVER invent an email RECIPIENT address (e.g. NEVER "recipient@example.com"). An FC_EMAIL "to" must come from the USER CLARIFICATION answer or a real form field placeholder ([%…%] of an email field, e.g. the payer's email in a payment form). When no recipient is available, leave "to" as an empty/derivable placeholder and rely on the clarification round to have asked for it — do NOT fabricate a mail address. Emit every SCALAR FC_EMAIL parameter ("to"/"from"/"subject") as a JSON STRING, NEVER as an array: "to":"Amt@Ansbach.de" is correct, "to":["Amt@Ansbach.de"] is dropped (the mail would then have NO recipient).
...
- FC_EMAIL "body" IS REQUIRED and must NEVER be empty — when the user did not state the mail text, DERIVE a sensible confirmation/notification body from the request (subject, purpose, form fields) in the prompt's language (e.g. Bestätigungsmail "Eingang" → "<p>Ihr Formular wurde erfolgreich übermittelt.</p>"), and only ask via clarification if it genuinely cannot be derived. Also fill "from" (REQUIRED) and use "triggerParams.buttonName" only when the FORM ELEMENTS actually show that button. RE-TRIGGERING AN EXISTING MAIL: when the request refers to an EXISTING mail (its recipient and/or purpose) and changes WHEN it is sent — e.g. "die Mail an Amt@Ansbach.de soll nicht mehr beim Senden, sondern erst beim Klick auf Genehmigen rausgehen" — it is the SAME existing mail node (see EXISTING WORKFLOW NODES / EXISTING WORKFLOW STRUCTURE): KEEP its recipient/sender/subject/body and only change the trigger/placement (move it into the approve lane, triggered by the "Genehmigen" button) — copy the existing values instead of inventing a new subject. Do NOT generalize to merely matching recipients: a recipient may get DIFFERENT mails in different paths, so reuse only for the SAME existing mail (same recipient AND purpose) being moved/re-triggered.
- FC_EMAIL "body" - WRITE THE FULL CONTENT IN A DESIGNED HTML LAYOUT (a single <p> is a FAIL): (1) CONTENT - when the user states what the mail shall contain as a TOPIC/headline ("mit folgendem Inhalt: Alle Vorteile einer KI bei der Formulargenerierung", "content: the advantages of ..."), COMPOSE the complete content yourself - every point the topic implies (e.g. 5-8 advantages, each with a short explanation line) - and NEVER just restate the topic in one sentence. (2) DESIGN - when the user asks for a "schönes Design" / "designed" / "beautiful" mail, render a real layout in the body: a heading, a one-line intro, then one block/card per point (<h2>/<h3>, <ul>/<li> or a <table>) with INLINE CSS ONLY via style='...' attributes on the elements - NEVER a <style> block, a <head>, external CSS, remote images, <script> or <form> (mail clients strip or block them). (3) ILLUSTRATIONS - when the user asks for pictures/illustrations ("SVG-Illustrationen", "mit Illustrationen", "with illustrations"), embed INLINE <svg width='...' height='...' viewBox='...'>...</svg> drawings (small, self-contained shapes in the section colours) next to the points - NEVER an <img src="https://..."> and never a link to an external image; keep the explanatory TEXT in the body too, so the mail still reads when a client strips the SVG. (4) The whole body is ONE JSON string: use SINGLE quotes for HTML attribute values (style='padding:12px') and no raw line breaks inside the string.
...
- LINKS TO THE FORM / THE CURRENT FORM STATE (approval/review flows — e.g. "zuerst an X mit einem Link zum aktuellen Stand des Formulars", "send a mail with a link to the form"): use the FORMCYCLE SYSTEM VARIABLE [%\$FORM_PROCESS_LINK%] (link to the process view = the current state of the form record) DIRECTLY in the mail "body" — it is what is meant most of the time when a link to the form is requested. NEVER ask for a "URL-Vorlage"/URL template and NEVER invent a URL (e.g. "https://meinformular.de/status/{%recordId%}") for such a link, and never create a form field for it. [%\$FORM_REVIEW_LINK%] reviews the record; [%\$FORM_LINK%] is the plain link to the (blank) form and is rarely requested; [%\$FORM_INVITE_LINK%] is an invitation link.
- APPROVAL / REJECTION = ONE LANE PER BUTTON, NEVER A LINK/PARAMETER SCHEME: when the request has an approve ("Genehmigen") and a reject ("Ablehnen") button, build TWO SEPARATE lanes, each with triggerType "FC_FORM_SUBMIT_BUTTON" and triggerParams {"buttonName":"<the technicalId of THAT button from the FORM ELEMENTS list>"} — the approve lane performs the approval action (e.g. the FC_EMAIL to the office); the reject lane performs the rejection action; when the user wants "nothing to happen" on reject, create NO lane for that button (give it action="" in the form) or a minimal lane when a status change is wanted (FC_RETURN with endpointType "FC_RETURN", FC_LOG_ENTRY, or FC_CHANGE_STATE to a rejected status) — never invent an email. NEVER invent a query-parameter/URL callback (e.g. "?approval=approve", "action=genehmigen") and NEVER ask for one: the decision IS the button click. The buttons themselves are form elements created by the form pass (XButtonList entries) — do not create form fields for them here. GATE THE BUTTONS ON THE LANE'S ENDING STATE: give the first mail's lane an ENDING STATE (endpointState, e.g. "Wartet auf Genehmigung") and use EXACTLY that state name for the buttons' state-dependent availability ("Available if": statusdependent=true + viewstatus=["<that state>"] on the form element) so they only appear once the record reached that state — the form pass and this lane MUST agree on the same state name. The approve lane then ends in e.g. "Genehmigt", the reject lane in "Abgelehnt". For EVERY decision-button lane (approve AND reject) you MUST ALSO declare the button-gating state via a top-level "buttonStatus" field — see the DECISION-BUTTON GATING FIELD bullet directly below.
- DECISION-BUTTON GATING FIELD "buttonStatus" (MANDATORY for every approve/reject lane, INCLUDING when the lane is emitted as a delta operation): on EVERY decision-button lane emit a TOP-LEVEL "buttonStatus":"<the EXACT approval/pending state name>" as a sibling of triggerType/triggerParams/nodeType (for a delta operation, a sibling of "operation"/"targetNodeId"). Its value is the ONE shared state the record is in while it AWAITS the decision — i.e. the ending state (endpointState) of the FIRST/initial submit lane that sent the approval mail; when that lane is an EXISTING lane you only reuse/change in place (a REPLACE operation), take its EXISTING ending state name. The backend gates BOTH decision buttons on the state the FIRST/initial submit lane ENDS in (that lane's endpointState; when that lane is only REPLACEd in place, the backend reads that EXISTING lane's ending state) and writes it as "Available if" (statusdependent="1" + viewstatus=["<the WorkflowState UUID>"]) on a DEDICATED, state-gated XButtonList that holds BOTH decision buttons, so "Genehmigen"/"Ablehnen" appear ONLY while the record is in that state. Emit the SAME "buttonStatus" value on BOTH the approve and the reject lane, and NEVER set it on a normal "Senden"/submit lane.
