...
- An element with NO `statusdependent`/`usergrouppendent` is available in every state (the default) — only set these when the element must appear in specific states.
