...
Rules:
- Emit the `*dependent` flag as the STRING `"1"` and the `*viewstatus`/`*viewusergroup` arrays as arrays of the workflow state UUIDs (plain strings) — any other shape (a string `"true"`, an object, a nested array) is dropped by the server.
- Reference the state the workflow lane ends in (endpointState) by its WorkflowState UUID in `viewstatus` - a state NAME never matches. For an approve/reject flow do NOT hand-write `viewstatus` at all: emit the pending state NAME in the workflow lane's `buttonStatus` and the backend resolves it to the state UUID (creating the state if needed).
- Typical approval flow: the submit lane sends the first mail and ends in a NEW state (e.g. "Wartet auf Genehmigung"); the decision buttons carry `"statusdependent":"1"` + `"viewstatus":["<the pending-state UUID>"]` (the backend applies this from the lane's `buttonStatus`) so they appear ONLY once the record reached that state; the data fields may carry `"readonly_statusdependent":true` + `"readonly_viewstatus":["Wartet auf Genehmigung"]` so the approver can only act on the buttons; the approve lane ends in "Genehmigt", the reject lane in "Abgelehnt".
