...
- JavaScript belongs to an ELEMENT, not to the form: a `<script>` is allowed ONLY inside the element it belongs to (that element's own HTML property, e.g. its `rtevalue`) and ONLY when the request asks for it. NEVER emit a form-level `<script>`, `<head>`, external CSS/remote images, and never an `<style>` block on a DIFFERENT element than the animated text. INTERACTIVE HTML in an element property (e.g. a calculator) MUST use `<button type='button'>` for every key — a bare `<button>` defaults to `type='submit'` and SUBMITS THE FORM when clicked — and its JavaScript must wire immediately (an IIFE, or ONE delegated listener on a stable `id`), NEVER via `DOMContentLoaded` (already fired when the HTML is inserted).
- a requested ILLUSTRATION / drawing (e.g. "eine animierte SVG Illustration … die ein Formular zeigt", "SVG", an illustration above the first line of the text) is written as an INLINE `<svg viewBox='…'>…</svg>` INSIDE the SAME `rtevalue` (NEVER an `<img src="https://…">`, never a remote image/animated GIF), placed at the requested position (e.g. as the FIRST element of the `rtevalue`, above the heading). It must be ANIMATED — SVG SMIL (`<animate attributeName='opacity' …/>`, `<animateTransform …/>`) AND/OR a CSS animation whose `@keyframes` you define in the in-`rtevalue` `<style>` block with the matching class on the SVG shape; a requested "animierte" illustration that renders static is a FAIL.
  ...
  - SINGLE QUOTES for EVERY SVG/HTML attribute value inside the `rtevalue` (`width='100%'`, `viewBox='0 0 W H'`, `fill='#4caf50'`) — an escaped double quote (`width=\"200\"`) produces a broken attribute, so the SVG can render at the wrong size or not at all. Keep the SVG itself self-contained: no external CSS/images/fonts, no `<foreignObject>` (a `<script>` may additionally sit in the element's own `rtevalue` when JavaScript was requested).
...
**XMap — the form assistant configures the widget in the FORM JSON AND, when the request asks for it, writes the JavaScript that syncs the drawn map geometry into a field INSIDE the element it belongs to.** Configure the widget via the xmap_ properties — `xmap_geometry_point` / `xmap_geometry_line` / `xmap_geometry_area` ("true" to allow that geometry) — and create the target field the geometry should go into (e.g. a number field `tfVeraeuss_Flaeche`). When the request needs the drawn area/line/point synced into that field, put the working script into the HTML-taking property of a form element (e.g. the `rtevalue` of an XSpan that is part of the map setup) — JavaScript always belongs to a single element, and there is NO form-level `_customScript` marker. Use the Formcycle/Leaflet API (`CRM.Helpers.getFieldValue`/`setFieldValue`, the XMap instance's geometry callbacks) to read the drawn geometry and write it into the target field.
