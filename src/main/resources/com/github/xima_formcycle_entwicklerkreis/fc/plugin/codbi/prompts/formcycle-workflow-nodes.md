...
AVAILABLE SERVER VARIABLES (system placeholders — use [%\$NAME%] syntax, no curly braces):
  ...
  A LINK TO THE FORM / TO THE CURRENT STATE OF THE FORM (approval/review mail "mit einem Link zum aktuellen Stand des Formulars") is [%\$FORM_PROCESS_LINK%] (process view) — use it DIRECTLY; it is what is meant most of the time when a link to the form is requested. [%\$FORM_REVIEW_LINK%] reviews the record; [%\$FORM_LINK%] is the plain (blank) form link (rarely requested); [%\$FORM_INVITE_LINK%] is an invitation link. NEVER ask for a URL template for such a link and NEVER invent a URL.
...
- CRITICAL — RE-TRIGGERING AN EXISTING MAIL (keep its subject/sender/body): when the request refers to an EXISTING mail (its recipient/purpose) and changes WHEN it is sent (e.g. not on submit anymore, but only when an approval button is clicked), it is the SAME existing mail node (EXISTING WORKFLOW NODES / EXISTING WORKFLOW STRUCTURE) — KEEP its sender/subject/body and only change the trigger/placement. Do NOT treat a merely MATCHING recipient as "the same mail" (a recipient can receive different mails in different paths) and never ask for the subject of an existing mail that is only being re-triggered.
...
- CONTENT - COMPOSE, DO NOT RESTATE: when the user gives the mail content as a topic/headline (e.g. "mit folgendem Inhalt: Alle Vorteile einer KI bei der Formulargenerierung"), write the FULL content about that topic (every point/section it implies, each with a short explanation) - a single <p> that merely restates the topic is a FAIL.
- DESIGN & ILLUSTRATIONS - for a requested "schönes Design" build a real layout in the body (heading + intro + one section/card per point, <h2>/<h3>, <ul>/<li>, <table>) using INLINE CSS only (style='...' attributes) - NEVER a <style> block, a <head>, external CSS, a remote <img src="https://...">, <script> or <form> (mail clients strip or block them). For requested illustrations ("SVG-Illustrationen") embed INLINE <svg width='...' height='...' viewBox='...'>...</svg> drawings (small, self-contained shapes) and keep the explanatory text in the body as well. Write HTML attribute values with SINGLE quotes so the body stays a valid JSON string.
...
- nodeParams: {"attributes":[{"name":"<key>","value":"<value>"},...], "writeAttributesToForm":<true|false>} — the value is set ONLY from this "attributes" array (never from a bare "attributeKey"/"value" pair).
- READABLE / EMAIL ACCUMULATION — to put ALL rows of a repeatable (dynamic) container into an email body / text as readable lines (NOT JSON), put the per-row FC_WRITE_FORM_RECORD_ATTRIBUTES INSIDE the FC_FOR_EACH_LOOP's nodeParams._childNodes and make its "value" CONCATENATE the previous value so the lines accumulate: value = "[%\$RECORD_ATTR.<key>%]" + this row's line, e.g. {"attributes":[{"name":"openingHours","value":"[%\$RECORD_ATTR.openingHours%][%selWeekday%]: [%tfFrom%] - [%tfTo%]\n"}]}. WITHOUT the leading [%\$RECORD_ATTR.<key>%] each iteration OVERWRITES the attribute and only the LAST row remains; then reference the completed [%\$RECORD_ATTR.<key>%] ONCE in the FC_EMAIL body after the loop.
