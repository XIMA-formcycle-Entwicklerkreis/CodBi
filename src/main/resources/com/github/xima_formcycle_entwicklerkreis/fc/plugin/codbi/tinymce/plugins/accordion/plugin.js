...
(function () {
  ...
  const fireToggleAllAccordionsEvent = (editor, elements, state) =>
  ...
  const insertAccordion = (editor) => {
    ...
    const summaryText = editor.dom.encode(
      editor.selection.getRng().toString() || editor.translate("Accordion summary..."),
    );
  ...
  const toggleAllAccordions = (editor, state) => {
    ...
    each$1(accordions, (accordion) =>
      toggleDetailsElement(accordion, state !== null && state !== void 0 ? state : !isOpen(accordion)),
    );
  ...
  const getClassList = (node) => {
    var _a, _b;
    return (_b = (_a = node.attr("class")) === null || _a === void 0 ? void 0 : _a.split(" ")) !== null && _b !== void 0
      ? _b
      : [];
  };
  ...
  const isAccordionDetailsNode = (node) =>
    node.name === accordionTag && contains(getClassList(node), accordionDetailsClass);
  const isAccordionBodyWrapperNode = (node) =>
    node.name === accordionBodyWrapperTag && contains(getClassList(node), accordionBodyWrapperClass);
  ...
  const register = (editor) => {
    ...
    editor.ui.registry.addContextToolbar("accordion", {
      predicate: (accordion) =>
        editor.dom.is(accordion, "details") &&
        editor.getBody().contains(accordion) &&
        editor.dom.isEditable(accordion.parentNode),
