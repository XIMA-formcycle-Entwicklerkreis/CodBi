...
(function () {
  ...
  const inList = (editor, parents, nodeName) =>
    findUntil(parents, (parent) => isListNode(parent) && !isCustomList(parent), isTableCellNode).exists(
      (list) => list.nodeName === nodeName && isChildOfBody(editor, list),
    );
  ...
  const addSplitButton = (editor, id, tooltip, cmd, nodeName, styles) => {
    ...
    editor.ui.registry.addSplitButton(id, {
      ...
      select: (value) => {
        ...
        return listStyleType.exists(
          (listStyle) =>
            value === listStyle || (listStyleTypeAliases[listStyle] === value && !stylesContainsAliasMap[value]),
        );
      },
