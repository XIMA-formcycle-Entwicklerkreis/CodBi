...
(function () {
  ...
  const is = (value, constructor) =>
    isObject(value) && hasProto(value, constructor, (o, proto) => getPrototypeOf(o) === proto);
  ...
  const collect = (editor) => {
    ...
    const futureImageList = new Promise((completer) => {
      createImageList(editor, (imageList) => {
        completer(urlListSanitizer(imageList).map((items) => flatten([[{ text: "None", value: "" }], items])));
      });
    });
  ...
  const register = (editor) => {
    ...
    editor.ui.registry.addContextMenu("image", {
      update: (element) =>
        editor.selection.isEditable() && (isFigure(element) || (isImage(element) && !isPlaceholderImage(element)))
          ? ["image"]
          : [],
    });
  };
