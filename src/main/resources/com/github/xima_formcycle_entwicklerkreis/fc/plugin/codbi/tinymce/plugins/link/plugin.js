...
(function () {
      ...
      processor: (value) => {
        ...
        if (valid) {
          if (value === true) {
            ...
            value === "http" /* AssumeExternalTargets.ALWAYS_HTTP */ ||
            value === "https" /* AssumeExternalTargets.ALWAYS_HTTPS */
          ) {
            return { value, valid };
          } else {
  ...
  const getHref = (elm) => {
    ...
    return (_b = (_a = elm.getAttribute("data-mce-href")) !== null && _a !== void 0 ? _a : elm.getAttribute("href")) !==
      null && _b !== void 0
      ? _b
      : "";
  };
  ...
  const isOnlyTextSelected = (editor) => {
    ...
    const isElement = (elm) =>
      elm.nodeType === 1 && !isAnchor(elm) && !has(inlineTextElements, elm.nodeName.toLowerCase());
  ...
  const handleExternalTargets = (href, assumeExternalTargets) => {
    if (
      (assumeExternalTargets === "http" /* AssumeExternalTargets.ALWAYS_HTTP */ ||
  ...
  const init = (initialData, linkCatalog) => {
    ...
    const getTitleFromUrlChange = (url) => {
      ...
      return someIf(
        ...
        Optional.from((_a = url.meta) === null || _a === void 0 ? void 0 : _a.title).getOr(""),
      );
    };
    const getTextFromUrlChange = (url) => {
      ...
      return someIf(
        ...
        Optional.from((_a = url.meta) === null || _a === void 0 ? void 0 : _a.text).getOr(url.value),
      );
    };
  ...
  const tryProtocolTransform = (assumeExternalTargets, defaultLinkProtocol) => (data) => {
    ...
    const suggestProtocol =
      (assumeExternalTargets === 1 /* AssumeExternalTargets.WARN */ && !hasProtocol(url)) ||
  ...
  const getLinkFromSelection = (editor) =>
    editor.selection.isCollapsed() || isSelectionOnImageWithEmbeddedLink(editor)
