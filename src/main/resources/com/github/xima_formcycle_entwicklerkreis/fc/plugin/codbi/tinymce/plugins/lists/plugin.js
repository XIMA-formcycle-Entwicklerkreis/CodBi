...
(function () {
  ...
  const isParentListSelected = (parentList, selectedBlocks) =>
    isNonNullable(parentList) && selectedBlocks.length === 1 && selectedBlocks[0] === parentList;
  ...
  const isListInsideAnLiWithFirstAndLastNotListElement = (list) =>
    parent(list).exists(
      (parent) =>
        isListItemNode(parent.dom) &&
  ...
  const getSelectedListRoots = (editor) => {
    ...
    return find(parentLists, (p) => isListInsideAnLiWithFirstAndLastNotListElement(SugarElement.fromDom(p))).fold(
  ...
  const inList = (parents, listName) =>
    findUntil(parents, isListNode, isTableCellNode).exists((list) => list.nodeName === listName && !isCustomList(list));
      ...
      each$1(composedLists, (composedList) => {
        fireListEvent(
          ...
          indentation === "Indent" /* Indentation.Indent */
            ? "IndentList" /* ListAction.IndentList */
            : "OutdentList" /* ListAction.OutdentList */,
          composedList.dom,
        );
      });
  ...
  const listToggleActionFromListName = (listName) => {
    switch (listName) {
      case "UL":
        return "ToggleUlList" /* ListAction.ToggleUlList */;
      case "OL":
        return "ToggleOlList" /* ListAction.ToggleOlList */;
      case "DL":
        return "ToggleDLList" /* ListAction.ToggleDLList */;
    }
  };
    ...
    const findBetterContainer = (container, forward) => {
      ...
      while ((node = walker[dir]())) {
        if (
          !(
            isVoid(editor, node) ||
            isZwsp(node.textContent) ||
            ((_a = node.textContent) === null || _a === void 0 ? void 0 : _a.length) === 0
          )
        ) {
    ...
    if (start && isTextNode$1(container)) {
      if (isZwsp(container.textContent)) {
        ...
        while (
          ...
          (isInline(editor, container.previousSibling) || isTextNode$1(container.previousSibling))
        ) {
    ...
    if (!start && isTextNode$1(container)) {
      if (isZwsp(container.textContent)) {
        ...
        while (
          ...
          (isInline(editor, container.nextSibling) || isTextNode$1(container.nextSibling))
        ) {
          container = container.nextSibling;
        }
      }
  ...
  const backspaceDelete = (editor, isForward) => {
    ...
    return (
      ...
      (selection.isCollapsed() ? backspaceDeleteCaret(editor, isForward) : backspaceDeleteRange(editor))
    );
  };
  ...
  const setupMenuButtonHandler = (editor, listName) => (api) => {
    const menuButtonHandler = (e) =>
      api.setEnabled(inList(e.parents, listName) && !isWithinNonEditableList(editor, e.element));
