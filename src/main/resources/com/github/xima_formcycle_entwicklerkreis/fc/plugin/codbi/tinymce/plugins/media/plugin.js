...
(function () {
  ...
  const getValue = (data, metaData, sourceInput) => (prop) => {
    ...
    const getFromMetaFirst = () =>
      getFromMetaData().orThunk(() =>
        getFromData().bind((child) => (isObject(child) ? getNonEmptyValue(child) : Optional.from(child))),
      );
  ...
  const isEmbedIframe = (url, mediaDataType) =>
    isNonNullable(mediaDataType) && mediaDataType === "ephox-embed-iri" && isNonNullable(matchPattern(url));
  const shouldInsertAsNewIframe = (prevData, newData) => {
    const hasDimensionsChanged = (prevData, newData) =>
  ...
  const showDialog = (editor) => {
    ...
    const handleUpdate = (api, sourceInput, prevData) => {
      ...
      const data =
        shouldInsertAsNewIframe(prevData, dialogData) && hasDimensions(editor)
          ? { ...dialogData, embed: "" }
          : dialogData;
  ...
  const register = (editor) => {
    ...
    editor.ui.registry.addToggleButton("media", {
      ...
      onSetup: (buttonApi) => {
        ...
        const unbindSelectorChanged = selection.selectorChangedWithUnbind(
