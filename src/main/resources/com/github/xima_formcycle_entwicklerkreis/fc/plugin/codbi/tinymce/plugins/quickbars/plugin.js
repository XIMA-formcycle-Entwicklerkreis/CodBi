    ...
    if (insertToolbarItems.length > 0) {
      editor.ui.registry.addContextToolbar("quickblock", {
        predicate: (node) => {
          ...
          return (
            !has$1(sugarNode, "data-mce-bogus") &&
            closest$1(sugarNode, 'table,[data-mce-bogus="all"]', isRoot).fold(
              () =>
                closest(
                  sugarNode,
                  (elem) => name(elem) in textBlockElementsMap && editor.dom.isEmpty(elem.dom),
                  isRoot,
                ),
              never,
            )
