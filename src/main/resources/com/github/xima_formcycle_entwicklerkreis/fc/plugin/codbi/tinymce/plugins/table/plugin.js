...
(function () {
  ...
  /*
    ...
    */
  const lift2 = (oa, ob, f) =>
    oa.isSome() && ob.isSome() ? Optional.some(f(oa.getOrDie(), ob.getOrDie())) : Optional.none();
  ...
  const getRowsFromSelection = (selected, selector) => {
    ...
    return lift2(cellOpt, rowsOpt, (cell, rows) =>
      filter$1(rows, (row) =>
        exists(fromDom(row.dom.cells), (rowCell) => get$2(rowCell, selector) === "1" || eq(rowCell, cell)),
      ),
    ).getOr([]);
  };
  ...
  const generateMenuItemsCallback = (editor, items, format, onAction) => (callback) =>
  ...
  const getClassList$1 = (editor) =>
    buildClassList(getCellClassList(editor)).map((items) => ({
      name: "class",
      type: "listbox",
      label: "Class",
      items,
  ...
  const getAlignment = (formats, formatName, editor, elm) =>
    find(formats, (name) => !isUndefined(editor.formatter.matchNode(elm, formatName + name))).getOr("");
  ...
  const extractDataFromSettings = (editor, hasAdvTableTab) => {
    ...
    const getBorder = () => {
      ...
      return get$3(attrs, "border").fold(
        () => ({}),
        (border) => ({ border }),
      );
    };
    ...
    const getCellPaddingCellSpacing = () => {
      const spacing = get$3(style, "border-spacing")
        .or(get$3(attrs, "cellspacing"))
        .fold(
          () => ({}),
          (cellspacing) => ({ cellspacing }),
        );
      const padding = get$3(style, "border-padding")
        .or(get$3(attrs, "cellpadding"))
        .fold(
          () => ({}),
  ...
  const getData$1 = (editor, cells) => {
    const cellsData = table(cells[0]).map((table) =>
      map(getSelectedCells(table, cells), (item) =>
        extractDataFromCellElement(editor, item.element, hasAdvancedCellTab(editor), item.column),
      ),
    );
  ...
  const getClassList = (editor) =>
    buildClassList(getRowClassList(editor)).map((items) => ({
      name: "class",
      type: "listbox",
      label: "Class",
      items,
  ...
  const getSelectionTargets = (editor) => {
    ...
    const findTargets = () =>
      getStart().bind((startCellOrCaption) =>
        flatten(
          lift2(table(startCellOrCaption), getEnd().bind(table), (startTable, endTable) => {
    ...
    const onSetupColumn = (lockedDisable) => (api) =>
      onSetup(api, (targets) => isCaption(targets.element) || isDisabledFromLocked(lockedDisable));
    ...
    const onSetupPasteableColumn = (getClipboardData, lockedDisable) => (api) =>
      onSetup(
        api,
        (targets) => isCaption(targets.element) || getClipboardData().isNone() || isDisabledFromLocked(lockedDisable),
      );
  ...
  const addButtons = (editor, selectionTargets) => {
    ...
    if (tableClassList.length !== 0 && editor.queryCommandSupported("mceTableToggleClass")) {
      editor.ui.registry.addMenuButton("tableclass", {
        ...
        fetch: generateMenuItemsCallback(editor, tableClassList, "tableclass", (value) =>
    ...
    if (tableCellClassList.length !== 0 && editor.queryCommandSupported("mceTableCellToggleClass")) {
      editor.ui.registry.addMenuButton("tablecellclass", {
        ...
        tooltip: "Cell styles",
        fetch: generateMenuItemsCallback(editor, tableCellClassList, "tablecellclass", (value) =>
  ...
  const addToolbars = (editor) => {
    const isEditableTable = (table) =>
      editor.dom.is(table, "table") && editor.getBody().contains(table) && editor.dom.isEditable(table.parentNode);
  ...
  const addMenuItems = (editor, selectionTargets) => {
    ...
    if (tableCellClassList.length !== 0 && editor.queryCommandSupported("mceTableCellToggleClass")) {
      editor.ui.registry.addNestedMenuItem("tablecellclass", {
        ...
        text: "Cell styles",
        getSubmenuItems: () =>
          buildMenuItems(editor, tableCellClassList, "tablecellclass", (value) =>
            editor.execCommand("mceTableCellToggleClass", false, value),
          ),
        onSetup: selectionTargets.onSetupCellOrRow,
      });
