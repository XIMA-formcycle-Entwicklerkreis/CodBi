...
export class MultiSelect extends Editors.BaseEditor<typeof MultiSelectType> {
  ...
  constructor(config: TEditorCfg<IMultiSelectDescriptor>) {
    ...
    for (const configuration of listing) {
      ...
      newElement.addEventListener("click", (event) => {
        ...
        Callbacks["set-property"].fire(this.property, newValue.join(), this);
      });
  ...
  override setValue(data: unknown): void {
    ...
    for (const standard of (data as string).split(",")) {
      const element: HTMLInputElement = this._element.querySelector(`[value="${standard.trim()}"]`) as HTMLInputElement;
